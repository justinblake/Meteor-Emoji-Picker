!function (f) {
  if ('object' == typeof exports && 'undefined' != typeof module) module.exports = f() else if ('function' == typeof define && define.amd) define([], f) else {
    var g
    g = 'undefined' != typeof window ? window : 'undefined' != typeof global ? global : 'undefined' != typeof self ? self : this, g.MeteorEmoji = f()
  }
}(function () {
  var define
  return function () {
      ...
      !function (global, factory) {
        ...
        var _createClass = function () {
          ...
          function MeteorEmoji() {
            _classCallCheck(this, MeteorEmoji), this.initiate()
          }

          return _createClass(MeteorEmoji, [{
            key: 'initiate', value: function () {
              var _this = this
            ...
            key: 'generateElements', value: function (emojiInput) {
              var clickLink = function (event) {
                if ($(emojiInput).parent('div').siblings('.cke').length) CKEDITOR.instances.bodyEditor.insertText(event.target.innerHTML) else {
                  var caretPos = emojiInput.selectionStart
                  null != caretPos ? (emojiInput.value = emojiInput.value.substring(0, caretPos) + ' ' + event.target.innerHTML + emojiInput.value.substring(caretPos), emojiInput.focus(), emojiInput.selectionStart = caretPos + 3) : $(emojiInput).append(event.target.innerHTML)
                }
                'undefined' != typeof angular && angular.element(emojiInput).triggerHandler('change')
              }, clickCategory = function (event) {
                emojiInput.selectionStart
                emojiPicker.style.display = 'block'
                var hideUls = emojiPicker.querySelectorAll('ul'), i = 1, l = hideUls.length
                for (i; i < l; i++) hideUls[i].style.display = 'none'
                var backgroundToggle = emojiPicker.querySelectorAll('.category a')
                for (i = 0, l = backgroundToggle.length, i; i < l; i++) backgroundToggle[i].style.background = 'none'
                emojiPicker.querySelector('.' + event.target.id).style.display = 'block', emojiPicker.querySelector('#' + event.target.id).style.background = '#c2c2c2'
              }
              ...
              facesCategory.style.padding = '10px 0px', facesCategory.style.margin = '0', facesCategory.style.listStyle = 'none', facesCategory.style.textAlign = 'left', facesCategory.style.marginLeft = '3%', facesCategory.classList.add('faces')
              var animalsCategory = document.createElement('ul')
              animalsCategory.style.padding = '10px 0px', animalsCategory.style.margin = '0', animalsCategory.style.listStyle = 'none', animalsCategory.style.textAlign = 'left', animalsCategory.style.marginLeft = '3%', animalsCategory.classList.add('animals'), animalsCategory.style.display = 'none'
              var foodCategory = document.createElement('ul')
              foodCategory.style.padding = '10px 0px', foodCategory.style.margin = '0', foodCategory.style.listStyle = 'none', foodCategory.style.textAlign = 'left', foodCategory.style.marginLeft = '3%', foodCategory.classList.add('food'), foodCategory.style.display = 'none'
              var sportCategory = document.createElement('ul')
              sportCategory.style.padding = '10px 0px', sportCategory.style.margin = '0', sportCategory.style.listStyle = 'none', sportCategory.style.textAlign = 'left', sportCategory.style.marginLeft = '3%', sportCategory.classList.add('sport'), sportCategory.style.display = 'none'
              var transportCategory = document.createElement('ul')
              transportCategory.style.padding = '10px 0px', transportCategory.style.margin = '0', transportCategory.style.listStyle = 'none', transportCategory.style.textAlign = 'left', transportCategory.style.marginLeft = '3%', transportCategory.classList.add('transport'), transportCategory.style.display = 'none'
              var objectsCategory = document.createElement('ul')
              objectsCategory.style.padding = '10px 0px', objectsCategory.style.margin = '0', objectsCategory.style.listStyle = 'none', objectsCategory.style.textAlign = 'left', objectsCategory.style.marginLeft = '3%', objectsCategory.classList.add('objects'), objectsCategory.style.display = 'none'
              ...
              var faces = [128513, 128514, 128515, 128516, 128517, 128518, 128521, 128522, 128523, 128524, 128525, 128527, 128530, 128531, 128532, 128534, 128536, 128538, 128540, 128541, 128542, 128544, 128545, 128546, 128547, 128548, 128549, 128552, 128553, 128554, 128555, 128557, 128560, 128561, 128562, 128563, 128565, 128567],
                animals = [128012, 128013, 128014, 128017, 128018, 128020, 128023, 128024, 128025, 128026, 128027, 128028, 128029, 128030, 128031, 128032, 128033, 128034, 128035, 128036, 128037, 128038, 128039, 128040, 128041, 128043, 128044, 128045, 128046, 128047, 128048, 128049, 128050, 128051, 128052, 128053, 128054, 128055, 128056, 128057, 128058, 128059, 128060],
                food = [127813, 127814, 127815, 127816, 127817, 127818, 127820, 127821, 127822, 127823, 127825, 127826, 127827, 127828, 127829, 127830, 127831, 127832, 127836, 127837, 127838, 127839, 127840, 127841, 127842, 127843, 127844, 127846, 127848, 127849, 127850, 127851, 127852, 127853, 127856, 127858, 127859, 127860, 127863, 127864, 127867],
                sport = [127921, 127923, 127934, 127935, 127936, 127937, 127938, 127939, 127940, 127942, 127944, 127946, 128675, 128692, 128693, 9917, 9918, 9978, 127907, 127919, 9971],
                transport = [128641, 128642, 128646, 128648, 128650, 128653, 128654, 128656, 128660, 128662, 128664, 128667, 128668, 128669, 128670, 128671, 128672, 128673, 128640, 128643, 128644, 128645, 128647, 128649, 128652, 128657, 128658, 128659, 128661, 128663, 128665, 128666, 128674, 128676, 128690],
                objects = [127890, 127880, 127881, 127887, 127891, 127905, 127906, 127908, 127909, 127911, 127912, 127915, 127916, 127918, 127919, 127926, 127927, 127928, 127929, 127930, 127931, 127932, 127968, 127973, 127978, 128147, 128148, 128149, 128150, 128151, 128152, 128187, 128186, 128197, 128213, 128247]
